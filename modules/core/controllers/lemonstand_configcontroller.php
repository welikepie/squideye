	...
	class LemonStand_ConfigController extends Phpr_Controller
	{
		...
		protected function mysql_onSave()
		{
			try
			{
				...
				$connection = @mysql_connect($this->validation->fieldValues['host'], 
					$this->validation->fieldValues['user'], 
					$this->validation->fieldValues['password']);

				if (!$connection)
					throw new Phpr_SystemException('Error connecting to MySQL server. MySQL error: '.mysql_error());
					
				$select_result = @mysql_select_db($this->validation->fieldValues['database']);
