<?
	...
	class Shop_Authorize_Net_Aim_Payment extends Shop_PaymentType
	{
		...
		public function update_customer_profile($host_obj, $customer, $data)
		{
			...
			$paymentProfile->billTo->firstName = $validation->fieldValues['FIRSTNAME'];
			$paymentProfile->billTo->lastName = $validation->fieldValues['LASTNAME'];
			$paymentProfile->billTo->company = $customer->company;
			$paymentProfile->billTo->address = $customer->billing_street_addr;
			$paymentProfile->billTo->city = $customer->billing_city;
			$paymentProfile->billTo->country = $customer->billing_country->name;
			if ($customer->billing_state)
				$paymentProfile->billTo->state = $customer->billing_state->code;
			$paymentProfile->billTo->zip = $customer->billing_zip;
			$paymentProfile->billTo->phoneNumber = $customer->phone;
			...
			$address->company = $customer->shipping_company;
			$address->address = $customer->shipping_street_addr;
			$address->city = $customer->shipping_city;
			$address->country = $customer->shipping_country->name;
			if ($customer->shipping_state)
				$address->state = $customer->shipping_state->code;
			$address->zip = $customer->shipping_zip;
			$address->phoneNumber = $customer->shipping_phone;
		...
		public function pay_from_profile($host_obj, $order, $back_end = false, $redirect = true)
		{
			...
			foreach ($order->items as $item)
			{
				...
				$lineItem->name = Phpr_Html::strTrim($item->product->name, 28);
				$lineItem->description = Phpr_Html::strTrim($product_name, 252);
