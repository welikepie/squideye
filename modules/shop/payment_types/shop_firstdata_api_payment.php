<?
	...
	class Shop_FirstData_Api_Payment extends Shop_PaymentType
	{
		...
		{
			$host_obj->add_field('server_url', 'Host Name', 'left')->tab('Configuration')->renderAs(frm_text)->comment('You can find the host name in the First Data/LinkPoint Welcome email message.', 'above', true)->validation()->fn('trim')->required('Please specify host name. The host name must start with <strong>https://</strong>', 'above', true);
			$host_obj->add_field('server_port', 'Port Number', 'right')->tab('Configuration')->renderAs(frm_text)->comment('You can find the port number in the First Data Welcome email message', 'above', true)->validation()->fn('trim')->required('Please specify port number.')->numeric();
		...
		private function post_data($host_obj, $document)
		{
			...
			$endpoint = $host_obj->server_url.':'.$host_obj->server_port;
