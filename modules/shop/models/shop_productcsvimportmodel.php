<?
	...
	class Shop_ProductCsvImportModel extends Backend_CsvImportModel
	{
		...
		protected function sanitize_product_url_name($name, $url_name, &$warnings, $line_number, $product_id)
		{
			...
			if (!preg_match('/^[0-9a-z_-]$/i', $url_name))
			{
				...
				if (!strlen($new_url_name))
					$new_url_name = $this->prepare_product_url_name($name);
				else
				{
					$product_filter = $product_id ? ' and id <> :id' : null;
		...
		{
			Db_DbHelper::query('delete from shop_tier_prices where product_id=:product_id', array('product_id'=>$product_id));
			$price_tiers = unserialize($price_tiers);
			if(count($price_tiers))
			{
				foreach($price_tiers as $price_tier)
				{
					...
					else Db_DbHelper::query('insert into shop_tier_prices(product_id, quantity, price) values (:product_id, :quantity, :price)', array('product_id'=>$product_id, 'customer_group_id'=>$price_tier->customer_group_id, 'quantity'=>$price_tier->quantity, 'price' => $price_tier->price));
				}
			}
		}
