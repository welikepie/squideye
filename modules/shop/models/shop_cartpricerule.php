<?php
	...
	class Shop_CartPriceRule extends Shop_PriceRuleBase
	{
		...
		public function eval_discount($cart_items, $current_subtotal, &$product_price_map, &$item_discount_tax_incl_map, $customer)
		{
			...
			return $this->get_action_obj()->eval_discount($params, $this, $product_price_map, $item_discount_tax_incl_map, $this->get_products_conditions_root(null, true));
		}
		...
		*/
		protected static function list_active_rules($coupon, $customer, $coupon_check = false)
		{
			...
			if($coupon_check)
				$rules_variable = 'return_rules';
			else
				$rules_variable = 'self::$active_rules';
			...
			foreach ($rules as $rule)
			{
				...
				array_push($$rules_variable, $rule);
			}

			return $$rules_variable;
		}
