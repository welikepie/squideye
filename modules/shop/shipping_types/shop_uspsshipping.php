	...
	class Shop_UspsShipping extends Shop_ShippingType
	{
		...
		*/
		...
		public function build_config_ui($host_obj, $context = null)
		{
			...
			$host_obj->add_field('domestic_allowed_services', 'Allowed domestic shipping services', 'left')->tab('Domestic Shipping')->renderAs(frm_checkboxlist)->validation();
			$host_obj->add_field('package_size', 'Package size', 'right')->tab('Domestic Shipping')->renderAs(frm_radio)->validation()->required('Please specify the package size');
		...
		public function get_package_size_options($current_key_value = -1)
		{
			$options = array(
				...
				'OVERSIZE'=>array('Oversize'=>'Length plus girth is more than 108 but not more than 130 inches')
			);
...
//			curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);
		...
		public function generate_shipping_labels($host_obj, $order, $parameters = array())
		{
			...
			if ($order->shipping_country->code_iso_numeric == 840 || $use_domestic)
			{
				...
				return array(
					new Shop_ShippingLabel($image_data, strtolower($image_type), $order, $host_obj)
				);
			}
