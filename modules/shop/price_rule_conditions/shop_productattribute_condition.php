	...
	class Shop_ProductAttribute_Condition extends Shop_ModelAttributesConditionBase
	{
		...
		public function is_true(&$params, $host_obj)
		{
			...
			if ($attribute != 'current_price' && $attribute != 'product' && $attribute != 'categories' && $attribute != 'on_sale' && $attribute != 'manufacturer_link')
				return parent::eval_is_true($params['product'], $host_obj);

			if ($attribute == 'on_sale')
			{
				if($params['product']->on_sale && !Shop_Product::is_sale_price_or_discount_invalid($params['product']->sale_price_or_discount) && strlen($params['product']->sale_price_or_discount))
				{
				...
				if (strlen($params['product']->price_rules_compiled))
				{
					try
					{
						$price_rules = unserialize($params['product']->price_rules_compiled);
						if ($price_rules)
