			...
			try
			{
				...
				if ($item && post('auto_discount_price_eval'))
				{
					...
					else
					{
						...
						$price = max($bundle_item_product->get_price_no_tax($product, $effective_quantity, $customer_group_id) - $product->get_discount($effective_quantity, $customer_group_id), 0);
					}
		...
		protected function onUpdateProductId($order_id)
		{
			try
			{
				...
				if (!$bundle_item_product_id)
					...
					$price = max(round($bundle_item_product->get_price_no_tax($item->product, $effective_quantity, $customer_group_id), 2) - $item->product->get_discount($effective_quantity, $customer_group_id), 0);
				}
